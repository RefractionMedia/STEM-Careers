...
namespace STEM_Careers.MobileAppService.Helpers
{
    public class WebContentHelper
    {
        ...
        public void GetPerson(People person)
        {
            if (person == null)
            {
            ...
            var articleBody = page.DocumentNode.Descendants().FirstOrDefault(x => x.Attributes.Contains("itemprop") && x.Attributes["itemprop"].ToString() == "articleBody");
            string str = "";
            foreach(var p in articleBody.Descendants())
            {
                if(p.Name == "p")
                {
                    str += p.InnerText.Normalize();
                }
            }
            person.Content = str;
            return;
        ...
        private void PeopleFromHtml(string html, string X = "", string field = "")
        {
            ...
            var str = File.Create("People.csv");
            var stream = new StreamWriter(str);
            var csvWriter = new CsvWriter(stream);
            csvWriter.WriteRecords<People>(people);
